      SUBROUTINE iterate(grid)
        ...
        INTEGER :: grid_size
        INTEGER :: x, y, z, w
        INTEGER :: x1, x2, y1, y2, z1, z2, w1, w2
        INTEGER :: next
        LOGICAL :: active

        grid_size = SIZE(grid, 1)
        ALLOCATE( scratch(grid_size, grid_size, grid_size, grid_size) )
        scratch = 0
          ...
          DO z = 1, grid_size
            DO y = 1, grid_size
              DO x = 1, grid_size
                ...
                w2 = MIN(w+1, grid_size)
                next = SUM(grid(x1:x2, y1:y2, z1:z2, w1:w2))
                next = next - grid(x, y, z, w)
                IF (grid(x, y, z, w) .EQ. 1) THEN
      ...
      PROGRAM part1
        ...
        DO index = 1, 20
          ...
          END IF
        END DO
        max_size = grid_size + (iterations * 2)
        midpoint = max_size / 2

        ALLOCATE( grid(max_size, max_size, max_size, max_size) )
        ...
        DO index = 1, grid_size
          ...
          DO inner = 1, grid_size
            ...
            IF (line(inner:inner) .EQ. '#') THEN
              grid(x, y, midpoint + 1, midpoint + 1) = 1
            END IF
          END DO
        END DO
